...
SimTransmissionConfig(tCar *car)
{
    ...
    if (strcmp(VAL_TRANS_RWD, transType) == 0) {
        ...
        finalRatio = trans->differential[TRANS_REAR_DIFF].ratio;
        ...
        finalRatio = trans->differential[TRANS_FRONT_DIFF].ratio;
        ...
        finalRatio = trans->differential[TRANS_CENTRAL_DIFF].ratio;
    ...
    for (i = MAX_GEARS - 1; i >= 0; i--) {
        ...
        carElt->priv.gearRatio[i] = trans->overallRatio[i] = gRatio * finalRatio;
        gEff = GfParmGetNum(hdle, path, PRM_EFFICIENCY, (char*)NULL, 1.0f);
        if (gEff > 1.0f) gEff = 1.0f;
        else if (gEff < 0.0f) gEff = 0.0f;
        gearI = GfParmGetNum(hdle, path, PRM_INERTIA, (char*)NULL, 0.0f);
        trans->driveI[i] = (car->engine.I + gearI) * (gRatio * gRatio * finalRatio * finalRatio);
        //printf ("drivetrain %d = %f %f\n", i, trans->driveI[i], gearI);
        trans->freeI[i] = gearI * (gRatio * gRatio * finalRatio * finalRatio);
...
SimTransmissionUpdate(tCar *car)
{
    ...
    switch(trans->type) {
        case TRANS_RWD:
            differential = &(trans->differential[TRANS_REAR_DIFF]);
            differential->in.Tq = transOutputTq; // (car->engine.Tq_response + car->engine.Tq) * trans->curOverallRatio * transfer * trans->gearEff[gearbox->gear+1];
